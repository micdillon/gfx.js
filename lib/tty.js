...
function Session(server, socket) {
  ...
  var static = '../static';
  if (conf.static) static = conf.static;
  static += '/data/';
  var self = this;
  ...
  watcher.on('add', function(filename) {
      ...
      if (fpath.match(/html$/)) {
          self.log('Watch Event [add] on file: %s -> rendering template into DOM.', fpath);
          self.socket.emit('data', -1, fpath);
      }
  });
  watcher.on('change', function(filename) {
      var fpath = '/data/' + path.basename(filename);
      if (fpath.match(/html$/)) {
          self.log('Watch Event [change] on file: %s -> rendering template into DOM.', fpath);
          self.socket.emit('data', -2, fpath);
      }
  });
}
